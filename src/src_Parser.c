...
/*
void getGlobList (char * pattern , glob_t * globList){
    glob(pattern,0,NULL, globList);
}
 */
...
data_t *parser(char *filename) {
	...
	char *path_sub = return_path_sub(filename);
		...
		switch (line[0]) {
			...
			case 'C':
				if (flag == JAVA) {
					...
					if (line[1] == 'H') {
						fprintf(stderr, "This command is not yet implemented, dumping: %s\n", line);
						exit(-95);
					}
